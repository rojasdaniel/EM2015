...
edad= [25, 40]
horita =[hora[1], hora[3]]
motivo = [motivos[2]]
estrato = [estratos[0], estratos[1], estratos[2]]
filtro = filtros[2]
...
def plot_station_counts(trip_counts):
    ...
    for index, row in trip_counts.iterrows():
        ...
        popup_text = "Barrio: {} <br> Numero salidas: {}<br> Numero de llegadas: {}<br> ZAT: {}"
    ...
    if count.empty:
        ...
        for k,v in dp.items():
            ...
            if tup in dp.keys(): 
                ...
                popup = "Origen: {} <br> Destino: {}"
                popup = popup.format(popup1, popup2)
                icon_url = 'https://worldtraveltoucan.com/wp-content/uploads/2017/11/destination-icon-300x300.png'
                icon = folium.features.CustomIcon(icon_url,icon_size=(38, 40))  
        ...
        for k,v in dp.items():
            if k[1]==k[0]:
                ...
                peso = sum(df.loc[(df['ZAT_ORIGEN'] == k[0]) & (df['ZAT_DESTINO'] == k[1]), 'Frecuencia de Viajes'])
